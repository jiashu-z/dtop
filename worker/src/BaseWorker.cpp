...
#define PRINT_ERROR(__WHEN__, __EXCEPTION__)                        \
  std::cout << "Error occur when " << (__WHEN__) << " in "          \
            << this->worker_name << " : " << (__EXCEPTION__).what() \
            << std::endl;
...
bool dtop::worker::BaseWorker::setup(
...
const std::vector<const dtop::worker::Future*>&
dtop::worker::BaseWorker::get_futures() const {
  return this->futures;
}
...
std::string dtop::worker::BaseWorker::get_status_str(
    dtop::worker::WorkerStatus status) {
  switch (status) {
    case NEW:
      return "NEW";
    case READY:
