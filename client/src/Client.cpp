...
std::string dtop::client::Client::to_string() { return "unimplemented!"; }

void dtop::client::Client::add_target(const std::string& addr) {
  std::string s = addr;
  ConcreteGRPCServiceClient client(
      grpc::CreateChannel(addr, grpc::InsecureChannelCredentials()));
  std::shared_ptr<ConcreteGRPCServiceClient> ptr =
      std::make_shared<ConcreteGRPCServiceClient>(
          grpc::CreateChannel(addr, grpc::InsecureChannelCredentials()));
  this->client_map.insert(
      std::pair<std::string, std::shared_ptr<ConcreteGRPCServiceClient>>(s,
                                                                         ptr));
}
