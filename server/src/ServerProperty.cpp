...
dtop::server::ServerProperty::ServerProperty(const std::string &config_file_path) {
...
void dtop::server::ServerProperty::config() {
  ...
  // Fetch properties.
  auto properties = dtop::common::check_and_fetch_xml_node(this->config_file, "properties");

  // Fetch server-addr.
  auto server_addr_node = dtop::common::check_and_fetch_xml_node(properties, "server-addr");
  ...
  // Fetch server-port.
  auto server_port_node = dtop::common::check_and_fetch_xml_node(properties, "server-port");
  this->port = std::stoi(server_port_node.child_value());

  // Fetch thread-num.
  auto thread_num_node = dtop::common::check_and_fetch_xml_node(properties, "thread-num");
...
std::string dtop::server::ServerProperty::get_ip() const {
  return this->ip;
}

int dtop::server::ServerProperty::get_port() const {
  return this->port;
}
